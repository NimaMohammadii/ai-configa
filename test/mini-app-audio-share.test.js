...
import { AI_CHAT_JS } from "../src/mini-app/ai-chat-client.js";
...
test("AI Chat renders credit errors from voice generation as chat messages", () => {
  assert.match(AI_CHAT_JS, /requestError\.status=response\.status/);
  assert.match(AI_CHAT_JS, /error\.status===402/);
});
