...
.vexa-live-balance{position:fixed;z-index:45;top:calc(12px + env(safe-area-inset-top));left:14px;height:36px;min-width:86px;padding:0 11px;${liquidGlassMaterialCss()}outline:0!important;border-radius:14px;color:#fff;display:inline-flex;align-items:center;justify-content:center;gap:6px;cursor:pointer;appearance:none;-webkit-appearance:none;box-shadow:inset 0 1px 0 rgba(255,255,255,.08),0 8px 24px rgba(0,0,0,.24);backdrop-filter:blur(10px) saturate(1.12);-webkit-backdrop-filter:blur(10px) saturate(1.12);transition:transform .28s cubic-bezier(.16,1,.3,1),background .25s ease,border-color .25s ease,box-shadow .25s ease;animation:vexaBalanceEnter .58s cubic-bezier(.16,1,.3,1) both}
.vexa-live-balance-value{font-size:12.5px;font-weight:760;line-height:1;letter-spacing:-.025em;font-variant-numeric:tabular-nums}
.vexa-live-balance-unit{color:rgba(255,255,255,.47);font-size:9px;font-weight:700;line-height:1;letter-spacing:.045em}
@media(hover:hover) and (pointer:fine){.vexa-live-balance:hover{transform:translateY(-1px);${LIQUID_GLASS_HOVER_CSS}}}
.vexa-live-balance:active{transform:scale(.94)}
.vexa-live-balance:focus-visible{outline:none!important;box-shadow:${LIQUID_GLASS_FOCUS_RING}!important}
...
.vexa-download-detail{color:rgba(255,255,255,.31);font-size:10px;font-weight:560;line-height:1.2;font-variant-numeric:tabular-nums;min-height:12px;transition:color .24s ease,font-weight .24s ease,transform .28s cubic-bezier(.16,1,.3,1)}
.vexa-download-detail.is-charge{color:rgba(255,255,255,.66);font-size:10.5px;font-weight:720;letter-spacing:-.006em;transform:translateY(-1px)}
...
@keyframes vexaBalanceEnter{from{opacity:0;transform:translateY(-7px) scale(.94)}to{opacity:1;transform:none}}
...
const SUBTITLE_PREFERENCE_SCRIPT = String.raw`(function(){
  ...
  const balanceButton=document.getElementById('vexaLiveBalance');
  const balanceValue=document.getElementById('vexaLiveBalanceValue');
  if(!root||!quality||!shell||!select||!button||!percent||!balanceButton||!balanceValue)return;
  ...
  function parentCreditPill(){try{return hostWindow().document.getElementById('creditPill');}catch(error){return null;}}
  function syncBalanceFromParent(){const pill=parentCreditPill();const text=String(pill?.textContent||'');const match=text.match(/\$?\s*\d[\d,]*(?:\.\d+)?/);balanceValue.textContent=match?String(match[0]).replace(/[$\s]/g,''):'—';}
  function openCredits(){haptic();const pill=parentCreditPill();if(pill){pill.click();return;}try{window.location.assign('/mini-app?section=bank_card');}catch(error){}}
  ...
  function haptic(){try{const host=hostWindow();(window.Telegram?.WebApp||host.Telegram?.WebApp)?.HapticFeedback?.impactOccurred?.('light');}catch(error){}}
  select.value='off';saveValue('off');select.addEventListener('change',function(){saveValue(String(select.value||'off'));haptic();});
  balanceButton.addEventListener('click',openCredits);const host=hostWindow();try{const pill=parentCreditPill();if(pill){const balanceObserver=new host.MutationObserver(syncBalanceFromParent);balanceObserver.observe(pill,{childList:true,subtree:true,characterData:true});}host.addEventListener('vexa:credits-balance',function(){requestAnimationFrame(syncBalanceFromParent);});}catch(error){}syncBalanceFromParent();
