...
html,body{margin:0;width:100%;height:100%;min-height:100%;overflow:hidden;overscroll-behavior:none;background:#07040d;color:#fff}
...
button{font:inherit}
.vexa-mesh-landing{position:fixed;inset:0;z-index:1;overflow:hidden;background:#07040d;opacity:1;transition:opacity .28s ease}
...
const FRAGMENT_SHADER = String.raw`// "Mesh drift" — made with the 21st.dev Shader Builder
...
function shader(type,source){const sh=gl.createShader(type);gl.shaderSource(sh,source);gl.compileShader(sh);if(!gl.getShaderParameter(sh,gl.COMPILE_STATUS)){const detail=gl.getShaderInfoLog(sh)||'Shader compile failed';gl.deleteShader(sh);throw new Error(detail);}return sh;}
function initShader(){
 if(!canvas)return;
 ...
 gl.uniform4f(gl.getUniformLocation(program,'u_cursor'),0.00,2.0,0.65,0.46);
 resize();start();
}
...
function mountVideo(data){
 ...
 let entered=false;
 const enter=function(){if(entered)return;entered=true;landing?.classList.add('is-hidden');stop();};
...
try{initShader();}catch(error){console.error('Vexa mesh shader failed',error);}
})();
`;
...
export async function appendVexaLiveLandingRuntime(request, response) {
  ...
  source = source.replace(/<meta name="theme-color" content="[^"]*" \/>/i, '<meta name="theme-color" content="#07040d" />');
