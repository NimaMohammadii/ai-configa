...
const RUNTIME_VERSION="20260820-13";
...
const INITIAL_AUDIO_BURST_SECONDS=2.4,MAX_AUDIO_LEAD_SECONDS=3.2;
...
function createRealtimeSubtitleSocket(request,env){
  ...
  const playbackControl={playbackTime:0,playbackRate:1,version:0,waiters:new Set()};
  let started=false;
  ...
  server.addEventListener("message",event=>{
    ...
    if(message?.type==="start"){
      ...
      runRealtimeSubtitleSession({
        request,env,server,payload:message,playbackStart,playbackControl,
        signal:controller.signal,send
      }).catch(fail);
      return;
    }
...
async function runRealtimeSubtitleSession({request,env,server,payload,playbackStart,playbackControl,signal,send}){
  ...
  scribeUrl.searchParams.set("vad_silence_threshold_secs","0.8");
  ...
  try{
    ...
    updatePlaybackControl(playbackControl,{currentTime:start,playbackRate});
    audioStream=await container.streamAudioPcm(playbackUrl,start,playbackRate,streamId);
    send({type:"ready"});
    await streamPcmToScribe({
      audioStream,upstream,control:playbackControl,baseStart:start,
      maxAheadSeconds:MAX_AUDIO_LEAD_SECONDS,signal
    });
...
async function waitForPlaybackLead(control,absoluteAudioTime,maxAheadSeconds,signal){
  while(!signal.aborted&&absoluteAudioTime-Number(control?.playbackTime||0)>Number(maxAheadSeconds||0)){
    ...
    await waitForControlChange(control,version,signal,1500).catch(error=>{
...
function patchClientRuntime(source){
  ...
  runtime=runtime.replace(
    ...
    'function sendPlaybackState(v){const current=socket;if(!current||current.readyState!==WebSocket.OPEN||!v)return;try{current.send(JSON.stringify({type:"playback_state",currentTime:Math.max(0,Number(v.currentTime||0)),playbackRate:Math.max(.25,Math.min(4,Number(v.playbackRate||1)))}));}catch{}}\nfunction restartFromCurrentTime(p){'
  );
  runtime=runtime.replace(
    'v.addEventListener("play",()=>{if(enabled)connectRealtime(p,v);});',
    'v.addEventListener("play",()=>{if(enabled){connectRealtime(p,v);sendPlaybackState(v);}});'
  );
  runtime=runtime.replace(
