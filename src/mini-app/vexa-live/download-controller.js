...
export async function appendVexaDownloadControllerRuntime(request, response) {
  ...
  const tag = '<script src="' + RUNTIME_PATH + '?v=20260829-reset-after-download-1"></script>';
...
(function(){
  ...
  function formatSubtitleCharge(value){const usd=Math.max(0,Number(value||0))*USD_PER_CREDIT;const digits=usd>0&&usd<.01?6:usd<1?4:2;return'$'+usd.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:digits});}
  function subtitleCompletionDetail(extra){const used=subtitleChargeCredits>0?'Used '+formatSubtitleCharge(subtitleChargeCredits):'';return[used,String(extra||'')].filter(Boolean).join(' · ');}
  ...
  function setState(state,message,detail){root.dataset.state=String(state||'idle');if(statusNode)statusNode.textContent=String(message||'');if(detailNode){const text=String(detail||'');detailNode.textContent=text;detailNode.classList.toggle('is-charge',text.startsWith('Used $'));}}
  ...
  function resetForNextDownload(){presetAvailable=false;provider='';sourceUrl='';downloadToken='';prepared=null;subtitleJob=false;subtitleResultUrl='';subtitleChargeCredits=0;nativeStarted=false;resultCheckInFlight=false;clearLocalFile();clearOptions();}
  ...
  if(preset.source){presetAvailable=false;setTimeout(function(){prepareSource(preset.source,preset.optionKey);},0);}else{setProgress(0,false);setState('idle','Ready when you are','');setButton('Download',false);setUploadDisabled(false);}
})();
`;
