...
function installStyles() {
  ...
  style.textContent =
    ...
    ".vexa-lm-content,.vexa-lm-inner-layer,.vexa-lm-shader-layer{position:absolute!important;inset:0!important;width:142px!important;height:46px!important;transform-style:preserve-3d!important;transition:all .8s cubic-bezier(.34,1.56,.64,1),box-shadow .15s cubic-bezier(.4,0,.2,1)!important}" +
    ...
    ".vexa-lm-inner{width:138px!important;height:42px!important;margin:2px!important;border-radius:100px!important;background:linear-gradient(180deg,#202020 0%,#000 100%)!important;box-shadow:none!important;transition:all .8s cubic-bezier(.34,1.56,.64,1),box-shadow .15s cubic-bezier(.4,0,.2,1)!important}" +
    ".vexa-lm-shader-layer{z-index:10!important;transform:translateZ(0) translateY(0) scale(1)!important}" +
    ".vexa-lm-shader-frame{width:142px!important;height:46px!important;border-radius:100px!important;background:rgb(0 0 0 / 0)!important;box-shadow:0 0 0 1px rgba(0,0,0,.3),0 36px 14px rgba(0,0,0,.02),0 20px 12px rgba(0,0,0,.08),0 9px 9px rgba(0,0,0,.12),0 2px 5px rgba(0,0,0,.15)!important;transition:all .8s cubic-bezier(.34,1.56,.64,1),box-shadow .15s cubic-bezier(.4,0,.2,1)!important}" +
    ".vexa-lm-shader{position:relative!important;width:142px!important;max-width:142px!important;height:46px!important;border-radius:100px!important;overflow:hidden!important;background:linear-gradient(115deg,#ececf0 0%,#353537 18%,#f7f7fa 36%,#18181a 56%,#e7e7eb 76%,#3b3b3e 100%)!important}" +
    ...
    "button.vexa-liquid-metal-button.is-pressed .vexa-lm-inner-layer{transform:translateZ(10px) translateY(1px) scale(.98)!important}button.vexa-liquid-metal-button.is-pressed .vexa-lm-shader-layer{transform:translateZ(0) translateY(1px) scale(.98)!important}" +
    "button.vexa-liquid-metal-button.is-pressed .vexa-lm-inner{box-shadow:inset 0 2px 4px rgba(0,0,0,.4),inset 0 1px 2px rgba(0,0,0,.3)!important}button.vexa-liquid-metal-button.is-pressed .vexa-lm-shader-frame{box-shadow:0 0 0 1px rgba(0,0,0,.5),0 1px 2px rgba(0,0,0,.3)!important}" +
    "button.vexa-liquid-metal-button.loading .vexa-lm-label{opacity:.5!important}button.vexa-liquid-metal-button:disabled .vexa-lm-stage{opacity:.55!important}" +
    ...
    ".tts-generate-row>.vexa-lm-shell{margin-top:0!important;margin-bottom:0!important}.image-workspace>.vexa-lm-shell{flex:0 0 46px!important;margin-top:0!important;margin-bottom:0!important}.vexa-stt-controls>.vexa-lm-shell{width:100%!important;min-width:0!important;display:flex!important;align-items:center!important;justify-content:center!important}" +
    "@keyframes vexaLmRipple{0%{transform:translate(-50%,-50%) scale(0);opacity:.6}100%{transform:translate(-50%,-50%) scale(4);opacity:0}}@media(prefers-reduced-motion:reduce){button.vexa-liquid-metal-button,.vexa-lm-content,.vexa-lm-inner-layer,.vexa-lm-shader-layer,.vexa-lm-inner,.vexa-lm-shader-frame{transition:none!important}.vexa-lm-ripple{animation:none!important}}";
...
function decorate(button, target) {
  ...
  button.addEventListener("pointerenter", function () {
    ...
    if (mount && mount.setSpeed) mount.setSpeed(HOVER_SPEED);
  });
  button.addEventListener("pointerleave", function () {
    button.classList.remove("is-hovered", "is-pressed");
    if (mount && mount.setSpeed) mount.setSpeed(BASE_SPEED);
  });
  button.addEventListener("pointerdown", function () {
    if (!button.disabled) button.classList.add("is-pressed");
  });
  function release() { button.classList.remove("is-pressed"); }
  button.addEventListener("pointerup", release);
  button.addEventListener("pointercancel", release);
  button.addEventListener("click", function (event) {
    if (button.disabled) return;
    addRipple(button, event);
    if (mount && mount.setSpeed) mount.setSpeed(CLICK_SPEED);
    window.setTimeout(function () {
      if (mount && mount.setSpeed) mount.setSpeed(button.classList.contains("is-hovered") ? HOVER_SPEED : BASE_SPEED);
    }, 300);
  });
  ...
  window.addEventListener("pagehide", function () { releaseMount(mount); }, { once: true });
}
