...
let timedSegments=[],renderTimer=0,stickyError='';
...
function clearCaptionState(p,clearError){timedSegments=[];clearRenderTimer();if(clearError!==false)stickyError='';hideCaption(p);}
function upsertSegment(message){
 ...
 const id=String(message.id||''),revision=Number(message.revision||0),segment={id,text,start,end,exact:Boolean(message.exact),revision};
 ...
 if(timedSegments.length>32)timedSegments=timedSegments.slice(-32);
 stickyError='';
 return true;
}
...
function nextBoundaryAfter(t,best){
 if(best&&best.end>t)return best.end+.19;
 for(let i=0;i<timedSegments.length;i++){const x=timedSegments[i];if(x.start>t+.12)return Math.max(t+.01,x.start-.1);}
 return null;
}
function scheduleRender(p,v){
 ...
 if(warmupActive){hideCaption(p);return;}
 const t=Math.max(0,Number(v?.currentTime||0));
 timedSegments=timedSegments.filter(function(x){return x.end>=t-.6&&x.start<=t+60;});
 const best=activeSegmentAt(t);
 ...
 const delay=Math.max(12,Math.min(60000,(boundary-t)*1000/rate));
...
function connectRealtime(p,v){
 ...
 ws.addEventListener('message',function(event){
   ...
   if(message.type==='audio_ready'){
     try{ws.send(JSON.stringify({type:'playback_start',currentTime:Math.max(0,Number(v.currentTime||0)),playbackRate:Math.max(.25,Math.min(4,Number(v.playbackRate||1))),playing:false,warmup:warmupActive}));}catch{}return;
   }
   if(message.type==='caption_segment'){
     if(upsertSegment(message))scheduleRender(p,v);
     return;
   }
   if(message.type==='translation_error'){
     setCaptionError(p,String(message.error||'Subtitle translation is temporarily unavailable'));return;
   }
 ...
 ws.addEventListener('close',function(){
   ...
   if(enabled&&!ws.__vexaIntentional&&!ws.__vexaFailed&&!v.ended)setCaptionError(p,'Live subtitles disconnected');
 });
...
function bindPlayer(p){
 ...
 v.addEventListener('pause',function(){if(!enabled)return;clearRenderTimer();if(warmupActive)return;sendPlaybackState(v,false);closeSocket();scheduleRender(p,v);});
 v.addEventListener('waiting',function(){if(enabled&&!warmupActive){sendPlaybackState(v,false);clearRenderTimer();}});
 v.addEventListener('stalled',function(){if(enabled&&!warmupActive){sendPlaybackState(v,false);clearRenderTimer();}});
 v.addEventListener('timeupdate',function(){if(enabled&&!warmupActive){maybeSyncPlayback(v);scheduleRender(p,v);}});
 v.addEventListener('seeking',function(){if(enabled){const resume=warmupActive?resumeAfterWarmup:Boolean(!v.paused&&!v.ended);warmupActive=true;resumeAfterWarmup=resume;p.classList.add('vexa-subtitle-warming');closeSocket();clearCaptionState(p,true);}});
