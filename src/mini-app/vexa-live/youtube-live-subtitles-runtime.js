...
function installUI(player){
 ...
 const layer=document.createElement('div');layer.className='vexa-subtitle-layer';layer.innerHTML='<div class="vexa-subtitle-frame" data-subtitle-frame><div class="vexa-subtitle-text" data-subtitle-text></div></div>';player.appendChild(layer);
...
function showCaption(p,text,error){const n=p.querySelector('[data-subtitle-text]'),frame=p.querySelector('[data-subtitle-frame]');if(!n||!frame)return;n.textContent=String(text||'');n.style.color=error?'#ffb1bd':'#fff';n.dir=targetLanguage==='fa'||targetLanguage==='ar'?'rtl':'auto';frame.classList.toggle('show',Boolean(text));}
function hideCaption(p){p.querySelector('[data-subtitle-frame]')?.classList.remove('show');}
