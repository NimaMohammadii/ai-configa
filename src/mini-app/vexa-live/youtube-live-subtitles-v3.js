...
import { LIVE_SUBTITLES_RUNTIME_JS as BASE_LIVE_SUBTITLES_RUNTIME_JS } from "./youtube-live-subtitles.js";

const SOCKET_PATH="/mini-app/live/api/youtube-subtitles/realtime";
const RUNTIME_PATH="/mini-app/vexa-live/live-subtitles.js";
const RUNTIME_VERSION="20260820-15";
...
const INITIAL_AUDIO_BURST_SECONDS=2.2,MAX_AUDIO_LEAD_SECONDS=2.4;
...
async function runRealtimeSubtitleSession({request,env,server,payload,playbackStart,playbackControl,signal,send}){
  ...
  let audioStream=null,translationLoopPromise=null,pendingFinal="",lastFinal="",upstreamEndedNormally=false,completed=false;
  ...
  const runTranslationLoop=()=>{
    translationLoopPromise=(async()=>{
      while(pendingFinal&&!signal.aborted&&server.readyState===WebSocket.OPEN){
        const sourceText=pendingFinal;
        pendingFinal="";
        try{
          if(targetLanguage==="original"){
            send({type:"caption",text:sourceText,translated:false});
          }else{
            const text=await translateSubtitle({env,sourceText,targetLanguage,signal});
            if(text&&!signal.aborted&&server.readyState===WebSocket.OPEN){
              send({type:"caption",text,translated:true});
            }
          }
        }catch(error){
          if(signal.aborted)return;
          console.error("Vexa final subtitle translation failed",error?.stack||error);
          send({type:"error",error:publicError(error)});
        }
      }
    })().finally(()=>{
      translationLoopPromise=null;
      if(pendingFinal&&!signal.aborted&&server.readyState===WebSocket.OPEN)return runTranslationLoop();
    });
    return translationLoopPromise;
  };

  const queueFinal=value=>{
    const sourceText=cleanSubtitleText(value);
    if(!sourceText||sourceText===lastFinal)return;
    lastFinal=sourceText;
    pendingFinal=sourceText;
  ...
  upstream.addEventListener("message",event=>{
    ...
    if(type==="final_transcript"){queueFinal(message?.text);return;}
    if(type==="committed_transcript"){queueFinal(message?.text);return;}
  ...
  try{
    ...
    updatePlaybackControl(playbackControl,{currentTime:start,playbackRate,playing:Boolean(playback.playing)});
  ...
  try{
    while(!signal.aborted){
      ...
      while(pending.byteLength>=PCM_FRAME_BYTES&&!signal.aborted){
        ...
        await waitForFeedPermission(control,absoluteAudioTime,maxAheadSeconds,signal);
...
async function waitForFeedPermission(control,absoluteAudioTime,maxAheadSeconds,signal){
  while(!signal.aborted&&(!control?.playing||absoluteAudioTime-Number(control?.playbackTime||0)>Number(maxAheadSeconds||0))){
    const version=Number(control?.version||0);
    await waitForControlChange(control,version,signal,1000);
  }
}
  ...
  try{
    const response=await fetch("https://api.openai.com/v1/responses",{
      ...
      body:JSON.stringify({
        ...
        instructions:"Translate this completed live subtitle segment into "+languageName+". Preserve the complete meaning, names, numbers and tone. Keep it concise enough for two subtitle lines. Return only the translation with no label, explanation, quotes or markdown.",
        input:sourceText,
...
function patchClientRuntime(source){
  ...
  runtime=runtime.replace(
    ...
    'ws.send(JSON.stringify({type:"playback_start",currentTime:Math.max(0,Number(v.currentTime||0)),playbackRate:Math.max(.25,Math.min(4,Number(v.playbackRate||1))),playing:playbackIsRunning(v)}));'
  );
  runtime=runtime.replace(
