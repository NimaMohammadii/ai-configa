...
} from "./youtube-live-subtitles-v4.js";
...
const RUNTIME_VERSION="20260820-17";
...
async function runRealtimeSubtitleSession({request,env,server,payload,playbackStart,playbackControl,signal,send}){
  ...
  const launchTranslation=()=>{
    ...
    task=(async()=>{
      try{
        ...
        if(job.revision<translationRevision||pendingJob){
          console.warn("Vexa skipped superseded live translation",error?.message||error);
          return;
        }
  ...
  const queueSource=value=>{
    ...
    pendingJob={text:sourceText,revision};
    scheduleTranslation();
  };
...
export const LIVE_SUBTITLES_RUNTIME_JS=BASE_LIVE_SUBTITLES_RUNTIME_JS;
