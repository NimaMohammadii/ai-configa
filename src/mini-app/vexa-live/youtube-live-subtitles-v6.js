...
import { LIVE_SUBTITLES_RUNTIME_JS as BASE_LIVE_SUBTITLES_RUNTIME_JS } from "./youtube-live-subtitles-v5.js";

const SOCKET_PATH="/mini-app/live/api/youtube-subtitles/realtime";
const RUNTIME_PATH="/mini-app/vexa-live/live-subtitles.js";
const RUNTIME_VERSION="20260820-18";
const TRANSLATION_MODEL="gpt-5.6-terra";
const TRANSLATE_TIMEOUT_MS=10000;
const TRANSLATION_MIN_INTERVAL_MS=150,TRANSLATION_MAX_CONCURRENCY=2;
const PCM_SAMPLE_RATE=16000,PCM_BYTES_PER_SECOND=32000,PCM_FRAME_BYTES=3200;
const INITIAL_AUDIO_BURST_SECONDS=5;
const INITIAL_AUDIO_LEAD_SECONDS=2.2,MIN_AUDIO_LEAD_SECONDS=1.2,MAX_AUDIO_LEAD_SECONDS=4.8;
const TRANSLATION_READY_MARGIN_MS=300;
...
function createRealtimeSubtitleSocket(request,env){
  ...
  const playbackControl={playbackTime:0,playbackRate:1,playing:false,version:0,waiters:new Set(),targetLeadSeconds:INITIAL_AUDIO_LEAD_SECONDS};
  let started=false;
...
async function runRealtimeSubtitleSession({request,env,server,payload,playbackStart,playbackControl,signal,send}){
  ...
  const audioClock={baseStart:0,lastSentTime:0};
  ...
  const scheduleTranslation=()=>{
    if(!pendingJob||translationTimer||signal.aborted||server.readyState!==WebSocket.OPEN)return;
    if(activeTranslations.size>=TRANSLATION_MAX_CONCURRENCY)return;
  ...
  const launchTranslation=()=>{
    if(!pendingJob||signal.aborted||server.readyState!==WebSocket.OPEN)return;
    if(activeTranslations.size>=TRANSLATION_MAX_CONCURRENCY)return;
    ...
    task=(async()=>{
      ...
      try{
        ...
        updateMeasuredLead(playbackControl,job.at,translationMs);
        logSubtitleTiming({job,translationMs,control:playbackControl});
        if(text&&job.revision>lastCaptionRevision&&!signal.aborted&&server.readyState===WebSocket.OPEN){
          lastCaptionRevision=job.revision;
          send({type:"caption",text,translated:true,at:job.at,revision:job.revision});
        }
      }catch(error){
    ...
    if(pendingJob)scheduleTranslation();
  };
  ...
  const queueSource=value=>{
    ...
    const at=finiteNumber(audioClock.lastSentTime,0,86400)??finiteNumber(playbackControl.playbackTime,0,86400)??0;
    if(targetLanguage==="original"){
      lastCaptionRevision=revision;
      send({type:"caption",text:sourceText,translated:false,at,revision});
      return;
    }
    pendingJob={text:sourceText,revision,at};
    scheduleTranslation();
  };
  ...
  upstream.addEventListener("error",()=>{if(signal.aborted)return;send({type:"error",error:"Realtime transcription connection failed"});try{server.close(1011,"scribe connection failed");}catch{}});
  upstream.addEventListener("close",()=>{if(signal.aborted||upstreamEndedNormally)return;send({type:"error",error:"Realtime transcription connection closed"});try{server.close(1011,"scribe connection closed");}catch{}});
  ...
  try{
    ...
    audioClock.baseStart=start;audioClock.lastSentTime=start;
    ...
    send({type:"ready"});
    await streamPcmToScribe({audioStream,upstream,control:playbackControl,baseStart:start,audioClock,signal});
    clearTranslationTimer();
    if(pendingJob)launchTranslation();
    if(activeTranslations.size)await Promise.allSettled([...activeTranslations]);
    if(!signal.aborted){completed=true;send({type:"ended"});}
  }finally{
    upstreamEndedNormally=true;
    clearTranslationTimer();pendingJob=null;
    signal.removeEventListener("abort",closeUpstream);signal.removeEventListener("abort",clearTranslationTimer);
...
async function streamPcmToScribe({audioStream,upstream,control,baseStart,audioClock,signal}){
  ...
  try{
    while(!signal.aborted){
      ...
      while(pending.byteLength>=PCM_FRAME_BYTES&&!signal.aborted){
        ...
        await waitForPlaybackLead(control,absoluteAudioTime,signal);
        ...
        bytesSent+=frame.byteLength;audioClock.lastSentTime=absoluteAudioTime;
      }
    }
    if(pending.byteLength&&!signal.aborted&&upstream.readyState===WebSocket.OPEN){
      ...
      audioClock.lastSentTime=absoluteAudioTime;
    }
  }finally{
...
function updatePlaybackControl(control,state){
  ...
  control.playing=Boolean(state?.playing);control.version+=1;notifyControl(control);
}
function updateMeasuredLead(control,sourceAt,translationMs){
  const current=finiteNumber(control?.targetLeadSeconds,MIN_AUDIO_LEAD_SECONDS,MAX_AUDIO_LEAD_SECONDS)??INITIAL_AUDIO_LEAD_SECONDS;
  const playbackAt=finiteNumber(control?.playbackTime,0,86400)??0;
  const readyAheadMs=(Number(sourceAt||0)-playbackAt)*1000;
  const computeNeed=Math.min(MAX_AUDIO_LEAD_SECONDS,Math.max(MIN_AUDIO_LEAD_SECONDS,(Number(translationMs||0)+TRANSLATION_READY_MARGIN_MS)/1000));
  const deadlineNeed=readyAheadMs<TRANSLATION_READY_MARGIN_MS?Math.min(MAX_AUDIO_LEAD_SECONDS,current+(TRANSLATION_READY_MARGIN_MS-readyAheadMs)/1000):computeNeed;
  const desired=Math.max(computeNeed,deadlineNeed);
  control.targetLeadSeconds=desired>current?desired:Math.max(MIN_AUDIO_LEAD_SECONDS,current*.92+desired*.08);
...
async function waitForPlaybackLead(control,absoluteAudioTime,signal){while(!signal.aborted&&absoluteAudioTime-Number(control?.playbackTime||0)>Number(control?.targetLeadSeconds||INITIAL_AUDIO_LEAD_SECONDS)){const version=Number(control?.version||0);await waitForControlChange(control,version,signal,1500);}}
...
function logSubtitleTiming({job,translationMs,control}){
  const playbackAt=finiteNumber(control?.playbackTime,0,86400)??0;
  console.log("Vexa subtitle timing",JSON.stringify({translation_ms:Math.round(Number(translationMs||0)),ready_ahead_ms:Math.round((Number(job?.at||0)-playbackAt)*1000),target_lead_ms:Math.round(Number(control?.targetLeadSeconds||0)*1000),revision:Number(job?.revision||0)}));
}
...
async function translateLiveSubtitle({env,sourceText,targetLanguage,signal}){
  ...
  try{
    ...
    throw error;
  }finally{clearTimeout(timer);signal.removeEventListener("abort",abortFromSession);}
}
async function readTranslationStream(response,signal){
  ...
  try{
    while(!signal.aborted){
      ...
      let match;
      while((match=/\r?\n\r?\n/u.exec(buffer))){const frame=buffer.slice(0,match.index);buffer=buffer.slice(match.index+match[0].length);consume(frame);}
      if(next.done)break;
    }
...
function publicScribeError(type,message){if(type==="quota_exceeded")return"Speech-to-text quota is unavailable";if(type==="rate_limited")return"Realtime transcription is rate limited";if(type==="auth_error")return"Realtime transcription authentication failed";if(type==="unaccepted_terms")return"Scribe realtime terms must be accepted";const detail=String(message?.error||message?.message||"");console.error("Vexa Scribe realtime error",type,detail.slice(0,700));return"Realtime transcription is temporarily unavailable";}
...
function patchClientRuntime(source){
  ...
  runtime=runtime.replace(
    ...
    'if(message.type==="caption"){queueTimedCaption(p,v,message);return;}'
  );
  runtime=runtime.replace(
    'function restartFromCurrentTime(p){',
    'const TIMED_CAPTION_STALE_SECONDS=.9,TIMED_CAPTION_MIN_GAP_SECONDS=.55,TIMED_CAPTION_HIDE_AFTER_SECONDS=3.2;\n'+
    ...
    'function queueTimedCaption(p,v,message){const text=String(message?.text||"").trim();if(!text)return;const at=Number(message?.at);if(!Number.isFinite(at)){showCaption(p,text,false);return;}const now=Math.max(0,Number(v?.currentTime||0));if(at<now-TIMED_CAPTION_STALE_SECONDS)return;const revision=Number(message?.revision||0);timedCaptionQueue.push({text,at,revision});timedCaptionQueue.sort((a,b)=>a.at-b.at||a.revision-b.revision);if(timedCaptionQueue.length>14)timedCaptionQueue=timedCaptionQueue.slice(-14);scheduleTimedCaptionPump(p,v);}\n'+
    ...
    'function pumpTimedCaptions(p,v,generation){if(generation!==timedCaptionPumpGeneration||!enabled||!v)return;const now=Math.max(0,Number(v.currentTime||0));while(timedCaptionQueue.length&&timedCaptionQueue[0].at<now-TIMED_CAPTION_STALE_SECONDS)timedCaptionQueue.shift();let candidate=null;while(timedCaptionQueue.length&&timedCaptionQueue[0].at<=now+.04)candidate=timedCaptionQueue.shift();if(candidate)timedCaptionDeferred=candidate;if(timedCaptionDeferred){const earliest=lastTimedCaptionMediaAt+TIMED_CAPTION_MIN_GAP_SECONDS;if(now+.04>=earliest){const item=timedCaptionDeferred;timedCaptionDeferred=null;if(now-item.at<=TIMED_CAPTION_STALE_SECONDS){showCaption(p,item.text,false);lastTimedCaptionMediaAt=now;timedCaptionHideAt=now+TIMED_CAPTION_HIDE_AFTER_SECONDS;}}}if(timedCaptionHideAt>0&&now>=timedCaptionHideAt&&!timedCaptionQueue.length&&!timedCaptionDeferred){p.querySelector("[data-subtitle-text]")?.classList.remove("show");timedCaptionHideAt=-1;}if(timedCaptionQueue.length||timedCaptionDeferred||timedCaptionHideAt>0)scheduleTimedCaptionPump(p,v);}\n'+
    'function restartFromCurrentTime(p){'
  );
  runtime=runtime.replace(
    'function hideCaption(p){p.querySelector("[data-subtitle-text]")?.classList.remove("show");}',
    'function hideCaption(p){clearTimedCaptions();p.querySelector("[data-subtitle-text]")?.classList.remove("show");}'
  );
  return runtime;
}
