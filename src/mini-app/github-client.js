...
(function(){
  ...
  async function loadRepositories(){var body=q('aiGithubBody');if(body)body.setAttribute('aria-busy','true');try{var data=await api('/mini-app/api/github/repositories',{});state.connected=!!data.connected;state.login=String(data.login||'');state.repository=data.repository||null;state.repositories=Array.isArray(data.repositories)?data.repositories:[];state.authorizeUrl=String(data.authorizeUrl||'');render()}catch(error){state.repositories=[];state.authorizeUrl='';render();showError(error.message)}finally{var currentBody=q('aiGithubBody');if(currentBody)currentBody.removeAttribute('aria-busy')}}
