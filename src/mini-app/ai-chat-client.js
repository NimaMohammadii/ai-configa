...
(function(){
  ...
  function startAiThinkingOrb(){stopAiThinkingOrb();var canvas=q('aiThinkingOrb');var emptyCanvas=q('aiChatEmptyOrb');if(!canvas&&!emptyCanvas)return;var row=q('aiThinkingRow');var initialState=row?row.getAttribute('data-state'):'';aiThinkingSearchMix=initialState==='searching'?1:0;aiThinkingVoiceMix=initialState==='generating_voice'?1:0;aiThinkingCodeMix=isAiCodingState(initialState)?1:0;aiThinkingCommitMix=isAiCommitState(initialState)?1:0;aiThinkingLastFrame=0;var reduced=window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)').matches;function frame(now){var seconds=(now||performance.now())/1000;var currentRow=q('aiThinkingRow');var state=currentRow?currentRow.getAttribute('data-state'):'';var searchTarget=state==='searching'?1:0;var voiceTarget=state==='generating_voice'?1:0;var codeTarget=isAiCodingState(state)?1:0;var commitTarget=isAiCommitState(state)?1:0;var delta=aiThinkingLastFrame?Math.min(.05,seconds-aiThinkingLastFrame):0;if(reduced){aiThinkingSearchMix=searchTarget;aiThinkingVoiceMix=voiceTarget;aiThinkingCodeMix=codeTarget;aiThinkingCommitMix=commitTarget}else{aiThinkingSearchMix=approachAiThinkingMix(aiThinkingSearchMix,searchTarget,delta,2.6);aiThinkingVoiceMix=approachAiThinkingMix(aiThinkingVoiceMix,voiceTarget,delta,1.15);aiThinkingCodeMix=approachAiThinkingMix(aiThinkingCodeMix,codeTarget,delta,3.2);aiThinkingCommitMix=approachAiThinkingMix(aiThinkingCommitMix,commitTarget,delta,3.8)}aiThinkingLastFrame=seconds;if(canvas)drawAiThinkingOrb(canvas,seconds,aiThinkingSearchMix,aiThinkingVoiceMix,aiThinkingCodeMix,aiThinkingCommitMix);if(emptyCanvas)drawAiThinkingOrb(emptyCanvas,seconds,0,0,0,0);var list=q('aiChatMessages');var emptyVisible=!!(aiChatOpen&&list&&!list.querySelector('.ai-chat-message,.ai-thinking-row'));if(!reduced&&(aiChatBusy||emptyVisible))aiThinkingFrame=requestAnimationFrame(frame)}frame(performance.now())}
  ...
  function renderAiChatStreamingMessage(stream){if(!stream||!stream.item||!stream.content)return;var value=String(stream.text||'');var rtl=/[\\u0590-\\u08ff\\ufb1d-\\ufdff\\ufe70-\\ufeff]/.test(value);stream.item.classList.toggle('rtl',rtl);stream.content.setAttribute('dir',rtl?'rtl':'ltr');renderAiChatMarkdown(stream.content,value)}
  function beginAiChatStreamingMessage(request){if(!request)return null;if(request.streaming)return request.streaming;var list=q('aiChatMessages');if(!list)return null;hideAiThinking();var item=document.createElement('article');item.className='ai-chat-message assistant ai-chat-streaming';var content=document.createElement('div');content.className='ai-chat-message-content';content.setAttribute('dir','ltr');item.appendChild(content);list.appendChild(item);var record={role:'assistant',content:''};aiChatMessages.push(record);request.streaming={item:item,content:content,record:record,text:'',frame:0};syncAiChatEmptyState();return request.streaming}
  function appendAiChatStreamingDelta(request,delta){if(!request||request.stopped||request.controller.signal.aborted||aiChatActiveRequest!==request)return;var value=String(delta||'');if(!value)return;var stream=request.streaming||beginAiChatStreamingMessage(request);if(!stream)return;stream.text+=value;stream.record.content=stream.text;if(!stream.frame){stream.frame=requestAnimationFrame(function(){stream.frame=0;renderAiChatStreamingMessage(stream);var list=q('aiChatMessages');if(list)list.scrollTop=list.scrollHeight})}}
  function removeAiChatStreamingMessage(request){var stream=request&&request.streaming;if(!stream)return;if(stream.frame){cancelAnimationFrame(stream.frame);stream.frame=0}var index=aiChatMessages.indexOf(stream.record);if(index>=0)aiChatMessages.splice(index,1);if(stream.item)stream.item.remove();request.streaming=null;syncAiChatEmptyState()}
  function finalizeAiChatStreamingMessage(request,finalText){var stream=request&&request.streaming;if(!stream)return false;if(stream.frame){cancelAnimationFrame(stream.frame);stream.frame=0}var value=finalText===undefined?String(stream.text||''):String(finalText||'');if(!value){removeAiChatStreamingMessage(request);return false}stream.text=value;stream.record.content=value;renderAiChatStreamingMessage(stream);stream.item.classList.remove('ai-chat-streaming');request.streaming=null;setAiChatCreatureState('happy');return true}
  ...
  function setAiThinkingState(payload){var row=q('aiThinkingRow');if(!row)return;var progress=payload&&typeof payload==='object'?payload:null;var state=progress?String(progress.state||'thinking'):String(payload||'thinking');var next='thinking';var labelText='Thinking';var detailText=progress?String(progress.detail||''):'';if(state==='searching'){next='searching';labelText='Searching…'}else if(state==='reading_repository'){next='reading_repository';labelText='Reading repository'}else if(state==='writing_code'||isAiCodingState(state)||isAiCommitState(state)){next=state;labelText='Writing code'}else if(state==='generating_voice'){next='generating_voice';labelText='Generating voice'}if(progress){next=state||next;labelText=String(progress.label||labelText);renderAiCodingProgress(progress)}setAiChatCreatureState(next==='searching'?'searching':'thinking');row.setAttribute('data-state',next);var label=row.querySelector('.ai-thinking-label');if(label)label.textContent=labelText;var detail=row.querySelector('.ai-thinking-detail');if(detail){detail.textContent=detailText;detail.classList.toggle('visible',!!detailText)}if(window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)').matches){var canvas=q('aiThinkingOrb');drawAiThinkingOrb(canvas,performance.now()/1000,next==='searching'?1:0,next==='generating_voice'?1:0,isAiCodingState(next)?1:0,isAiCommitState(next)?1:0)}}
  ...
  function setAiChatRequestUi(active){var send=q('aiChatSend');var attach=q('aiChatAttach');if(send){if(!aiChatSendIconMarkup)aiChatSendIconMarkup=send.innerHTML;send.disabled=false;send.classList.toggle('is-stop',!!active);send.setAttribute('aria-label',active?'Stop generating':'Send message');send.innerHTML=active?'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden="true"><rect x="7" y="7" width="10" height="10" rx="2.2" fill="currentColor"/></svg>':aiChatSendIconMarkup}if(attach)attach.disabled=!!active}
  function releaseAiChatRequest(request){if(aiChatActiveRequest!==request)return;aiChatActiveRequest=null;aiChatBusy=false;setAiChatRequestUi(false)}
  function stopAiChatGeneration(){var request=aiChatActiveRequest;if(!aiChatBusy||!request)return;request.stopped=true;aiChatSendKeepsKeyboard=false;request.controller.abort('user_stop');hideAiThinking();hideAiImageGenerating();finalizeAiChatStreamingMessage(request);releaseAiChatRequest(request);var input=q('aiChatInput');if(input)input.focus();if(tg&&tg.HapticFeedback){try{tg.HapticFeedback.impactOccurred('light')}catch(error){}}}
  async function sendAiChat(){if(aiChatBusy)return;var input=q('aiChatInput');var keepKeyboard=aiChatSendKeepsKeyboard||!!(input&&document.activeElement===input);aiChatSendKeepsKeyboard=false;if(keepKeyboard&&input)input.focus();var typed=String(input&&input.value||'').trim();var attachment=aiChatAttachment;if(!typed&&!attachment)return;var message=typed||(attachment.isImage?'What is in this image?':'Analyze this attachment.');if(attachment){aiChatMessages.forEach(function(item){if(item&&item.attachment)delete item.attachment})}appendAiChatMessage('user',message,false,attachment);clearAiChatAttachment();if(input){input.value='';resizeAiChatInput()}var request={controller:new AbortController(),stopped:false,streaming:null};aiChatActiveRequest=request;aiChatBusy=true;setAiChatRequestUi(true);showAiThinking();try{var requestMessages=aiChatMessages.slice(-20).map(function(item){return Object.assign({},item)});var latestRequest=requestMessages[requestMessages.length-1];if(latestRequest&&latestRequest.role==='user')latestRequest.preferredVoice=aiChatPreferredVoice;var data=await streamAiChat({messages:requestMessages},setAiThinkingState,function(delta){appendAiChatStreamingDelta(request,delta)},request.controller.signal);if(request.stopped||request.controller.signal.aborted||aiChatActiveRequest!==request)return;updateAiChatHeader(data);if(data.type==='image_request'){removeAiChatStreamingMessage(request);hideAiThinking();showAiImageGenerating();var imageData=await api('/mini-app/api/image',{prompt:String(data.prompt||message),size:String(data.size||'1024x1024')},request.controller.signal);if(request.stopped||request.controller.signal.aborted||aiChatActiveRequest!==request)return;hideAiImageGenerating();imageData.prompt=String(data.prompt||message);updateAiChatHeader(imageData);appendAiChatImage(imageData)}else if(data.type==='speech_request'){removeAiChatStreamingMessage(request);setAiThinkingState('generating_voice');var audioData=await api('/mini-app/api/tts',{text:String(data.text||''),voice:String(data.voice||'Nora')},request.controller.signal);if(request.stopped||request.controller.signal.aborted||aiChatActiveRequest!==request)return;hideAiThinking();audioData.text=String(data.text||'');updateAiChatHeader(audioData);appendAiChatAudio(audioData)}else{hideAiThinking();var streamed=finalizeAiChatStreamingMessage(request,String(data.message||''));releaseAiChatRequest(request);appendAiCodingResult(data.codingActivity);if(!streamed)await appendAiChatMessage('assistant',String(data.message||''),true)}}catch(error){if(request.stopped||request.controller.signal.aborted)return;hideAiThinking();hideAiImageGenerating();var hadPartial=finalizeAiChatStreamingMessage(request);var errorMessage=error.message||'Could not reach AI';if(error.status===402||String(errorMessage).indexOf('Not enough credits')===0){if(hadPartial)toast(errorMessage);else await appendAiChatMessage('assistant',String(errorMessage),true)}else toast(errorMessage)}finally{releaseAiChatRequest(request)}}
  async function streamAiChat(body,onStatus,onDelta,signal){var response;try{response=await fetch('/mini-app/api/chat',{method:'POST',headers:{'content-type':'application/json','accept':'application/x-ndjson'},cache:'no-store',signal:signal,body:JSON.stringify(Object.assign({initData:initData},body||{}))})}catch(error){if(signal&&signal.aborted)throw error;throw new Error('Connection interrupted · Try again')}if(!response.ok){var failed=await response.json().catch(function(){return{error:'Request failed'}});throw new Error(failed.error||'Request failed')}if(!response.body)throw new Error('Invalid response');var reader=response.body.getReader();var decoder=new TextDecoder();var buffer='';var result=null;var newline=String.fromCharCode(10);function consume(line){var clean=String(line||'').trim();if(!clean)return;var event;try{event=JSON.parse(clean)}catch(error){return}if(event.type==='status'||event.type==='progress'){if(typeof onStatus==='function')onStatus(event.type==='progress'?event.data:event.status);return}if(event.type==='delta'){if(typeof onDelta==='function')onDelta(String(event.delta||''));return}if(event.type==='error')throw new Error(event.error||'Could not reach AI');if(event.type==='result')result=event.data}while(true){var chunk=await reader.read();buffer+=decoder.decode(chunk.value||new Uint8Array(),{stream:!chunk.done});var index=buffer.indexOf(newline);while(index>=0){consume(buffer.slice(0,index));buffer=buffer.slice(index+1);index=buffer.indexOf(newline)}if(chunk.done)break}if(buffer.trim())consume(buffer);if(!result)throw new Error('Invalid response');return result}
  ...
  window.addEventListener('pagehide',function(){if(aiChatActiveRequest){aiChatActiveRequest.stopped=true;aiChatActiveRequest.controller.abort('pagehide')}stopAiChatVoicePreview();aiChatAudioUrls.forEach(function(url){URL.revokeObjectURL(url)});aiChatAudioUrls=[]});
