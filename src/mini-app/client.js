...
(function(){
  ...
  function applyVoiceProfiles(profiles){profiles=profiles||{};document.querySelectorAll('.voice-option').forEach(function(row){var button=row.querySelector('.voice-select[data-voice-name]');var image=row.querySelector('.voice-avatar-image');if(!button||!image)return;var name=button.getAttribute('data-voice-name')||'';var url=profiles[name];if(url){image.style.backgroundImage='url("'+String(url).replace(/"/g,'%22')+'")';image.classList.add('has-image')}else{image.style.backgroundImage='';image.classList.remove('has-image')}})}
