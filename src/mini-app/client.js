...
(function(){
  ...
  function setWavePlaying(playing){var button=q('wavePlay');if(!button)return;button.textContent=playing?'■':'▶';button.setAttribute('aria-label',playing?'Stop audio':'Play audio')}
  ...
  function playTts(){var audio=q('ttsAudio');if(!audio||!audio.src)return toast('Generate voice first');stopPreview();if(audio.paused){audio.play().catch(function(error){toast(error.message)})}else{audio.pause();audio.currentTime=0;setText('waveTime','0:00')}}
