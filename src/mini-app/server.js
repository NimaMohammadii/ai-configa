...
const APP_MODE_SETTINGS_PATCH = String.raw`
  ...
  function appModeRender(){var root=document.getElementById('vexaAppModeSettings');if(!root)return;root.dir=['fa','ar'].indexOf(appModeLanguage)>=0?'rtl':'ltr';var rows=root.querySelector('#vexaAppModeRows');if(!rows)return;rows.innerHTML=appModeModes.filter(function(item){return['tts','image','ai_chat','live'].indexOf(item.id)>=0}).map(function(item){var active=item.id===appModeValue;var locked=!!appModeLocks[item.id];return '<button class="vexa-app-mode-row" data-app-mode="'+item.id+'" role="switch" aria-checked="'+(active?'true':'false')+'" type="button"'+((appModeSaving||locked)?' disabled':'')+'><span class="vexa-app-mode-copy"><strong>'+item.title+'</strong><small>'+item.note+'</small></span><span class="vexa-app-mode-switch" aria-hidden="true"><i></i></span></button>'}).join('')}
