...
async function handleAppModeUpdate(request, env) {
  try {
    ...
    if ((await getAppModeLocks(env))[requestedMode] && !(await isAdmin(env, user.id))) {
      return json({ error: "This section is locked by the administrator." }, 423);
    }
...
function applyPrimaryAppStartup(source) {
  const replacements = [
    [
      "if(tg){try{tg.ready&&tg.ready();tg.expand&&tg.expand();",
      "if(tg){try{tg.expand&&tg.expand();",
    ],
    [
      "startAiChatButtonOrb();initializeDemoLanguagePicker();initializeWaveSeek();initializeDialogueEditor();setWavePlaying(false);updateTtsCharCount();updateImagePromptCount();updateImageCreditNote();selectImageSize('1024x1024');setCreationMode('voice');loadImageHistory();load();",
      "startAiChatButtonOrb();initializeDemoLanguagePicker();initializeWaveSeek();initializeDialogueEditor();setWavePlaying(false);updateTtsCharCount();updateImagePromptCount();updateImageCreditNote();selectImageSize('1024x1024');loadImageHistory();load();",
    ],
  ];
  ...
  for (const [before, after] of replacements) {
    if (!patched.includes(before)) {
      console.error("primary app startup patch target missing");
      continue;
    }
...
function applyAiChatPrimaryNavigation(source) {
  const before = "if(tg&&tg.BackButton){try{tg.BackButton.show();tg.BackButton.onClick(closeAiChat)}catch(e){}}";
  const after = "if(tg&&tg.BackButton){try{tg.BackButton.hide()}catch(e){}}";
  if (!source.includes(before)) {
    console.error("AI Chat primary navigation patch target missing");
    return source;
  }
  return source.replace(before, () => after);
}
...
async function stripCreditsHeaderCopy(response) {
  ...
  const source = await response.text();
  let html = source.replace("<p>Top up instantly and keep creating.</p>", "");
  const startupFlow = '<section id="flow" class="view active">';
  if (!html.includes(startupFlow)) {
    console.error("primary app startup flow target missing");
  } else {
    html = html.replace(startupFlow, '<section id="flow" class="view">');
  }
...
const APP_MODE_SETTINGS_PATCH = String.raw`
  ...
  var appModeCurrent=window.location.pathname.indexOf('/mini-app/chat')===0?'ai_chat':'tts';
  var appModeLanguage='en';
  var appModeSaving=false;
  var appModeLocks={};
  var appModeReady=false;
  var appModeStartupFinished=false;
  var appModeSettingsReturn='';
  var appModeBaseFetch=window.fetch.bind(window);
  var appModeObserver=null;
  var appModeBackBound=false;
  var appModeControlsObserver=null;
  var appModeModes=[
    {id:'tts',title:'Text to Speech',note:'AI voice from text'},
    {id:'image',title:'Image Creator & Explore',note:'Create images and explore ready-made prompts'},
  ...
  function appModeReferralMode(raw){var match=String(raw||'').match(/^ref_\d+_([tixc])$/i);if(!match)return'';return{t:'tts',i:'image',x:'explore',c:'ai_chat'}[String(match[1]).toLowerCase()]||''}
  function appModeIsSettingsOpen(){var root=document.getElementById('vexaAppModeSettings');return!!(root&&root.classList.contains('open'))}
  ...
  function appModeWhen(selector,callback){return new Promise(function(resolve){var finish=function(node){try{callback(node)}finally{resolve()}};var node=document.querySelector(selector);if(node){finish(node);return}if(appModeObserver)appModeObserver.disconnect();appModeObserver=new MutationObserver(function(){var found=document.querySelector(selector);if(!found)return;appModeObserver.disconnect();appModeObserver=null;finish(found)});appModeObserver.observe(document.documentElement,{childList:true,subtree:true})})}
  function appModeFinishStartup(showFlow){if(appModeStartupFinished)return;appModeStartupFinished=true;if(showFlow!==false){var flow=document.getElementById('flow');if(flow)flow.classList.add('active')}try{appModeTg&&appModeTg.ready&&appModeTg.ready()}catch(e){}}
  function appModeSyncBack(){if(!appModeTg||!appModeTg.BackButton)return;try{if(appModeIsSettingsOpen()||(appModeReady&&appModeCurrent!==appModeValue))appModeTg.BackButton.show();else appModeTg.BackButton.hide()}catch(e){}}
  function appModeBackPressed(){if(appModeIsSettingsOpen()){appModeCloseSettings();return}if(appModeReady&&appModeCurrent!==appModeValue)appModeApply(appModeValue)}
  function appModeBindBack(){if(appModeBackBound||!appModeTg||!appModeTg.BackButton)return;try{appModeTg.BackButton.onClick(appModeBackPressed);appModeBackBound=true}catch(e){}}
  function appModeSetCurrent(mode){appModeCurrent=appModeNormalize(mode);appModeSyncBack();requestAnimationFrame(appModeSyncBack)}
  function appModeCurrentFromHeader(){var live=document.getElementById('vexaLiveOpen');if(live&&live.getAttribute('aria-pressed')==='true')return'live';var stt=document.getElementById('speechToTextOpen');if(stt&&stt.getAttribute('aria-pressed')==='true')return'stt';return document.body&&document.body.classList.contains('image-mode')?'image':'tts'}
  function appModeBindHeaderButton(id){var button=document.getElementById(id);if(!button||button.dataset.vexaAppModeBound==='1')return;button.dataset.vexaAppModeBound='1';button.addEventListener('click',function(){setTimeout(function(){appModeSetCurrent(appModeCurrentFromHeader())},0)})}
  function appModeBindExistingControls(){['modeToggle','vexaLiveOpen','speechToTextOpen'].forEach(appModeBindHeaderButton);if(appModeControlsObserver||!document.documentElement)return;appModeControlsObserver=new MutationObserver(function(){['modeToggle','vexaLiveOpen','speechToTextOpen'].forEach(appModeBindHeaderButton)});appModeControlsObserver.observe(document.documentElement,{childList:true,subtree:true})}
  function appModeCloseWorkspaces(target){try{if(target!=='explore'&&typeof setExplorePage==='function')setExplorePage(false)}catch(e){}var live=document.getElementById('vexaLiveOpen');if(target!=='live'&&live&&live.getAttribute('aria-pressed')==='true')try{live.click()}catch(e){}var stt=document.getElementById('speechToTextOpen');if(target!=='stt'&&stt&&stt.getAttribute('aria-pressed')==='true')try{stt.click()}catch(e){}}
  async function appModeApply(mode){mode=appModeNormalize(mode);var chatPath=window.location.pathname.indexOf('/mini-app/chat')===0;if(chatPath){if(mode==='ai_chat'){appModeSetCurrent('ai_chat');return true}window.location.replace('/mini-app?section='+encodeURIComponent(mode));return false}if(mode==='ai_chat'){window.location.replace('/mini-app/chat');return false}appModeCloseWorkspaces(mode);if(mode==='tts'){try{if(typeof setCreationMode==='function')setCreationMode('voice');else if(document.body&&document.body.classList.contains('image-mode'))appModeClick('toggle-creation-mode')}catch(e){}appModeSetCurrent(mode);return true}if(mode==='image'){try{if(typeof setCreationMode==='function')setCreationMode('image');else if(document.body&&!document.body.classList.contains('image-mode'))appModeClick('toggle-creation-mode')}catch(e){}appModeSetCurrent(mode);return true}if(mode==='explore'){try{if(typeof setCreationMode==='function')setCreationMode('image');else if(document.body&&!document.body.classList.contains('image-mode'))appModeClick('toggle-creation-mode')}catch(e){}try{if(typeof openExplorePage==='function')openExplorePage();else await appModeWhen('[data-action="open-explore-page"]',function(button){button.click()})}catch(e){}appModeSetCurrent(mode);return true}if(mode==='stt'){await appModeWhen('#speechToTextOpen',function(button){if(button.getAttribute('aria-pressed')!=='true')button.click();appModeSetCurrent('stt')});return true}if(mode==='live'){await appModeWhen('#vexaLiveOpen',function(button){if(button.getAttribute('aria-pressed')!=='true')button.click();appModeSetCurrent('live')});return true}return true}
  async function appModeApplySession(data){if(!data||typeof data!=='object')return;if(data.locked){appModeFinishStartup(false);return}appModeLocks=data.appModeLocks||{};appModeValue=appModeNormalize(data.appMode);appModeLanguage=String(data.language||appModeLanguage||'en').toLowerCase().split(/[-_]/)[0];appModeReady=true;appModeInstallStyle();appModeBindBack();appModeBindExistingControls();appModeRender();var raw=appModeRawLaunch();var chatPath=window.location.pathname.indexOf('/mini-app/chat')===0;if(chatPath){appModeSetCurrent('ai_chat');if(raw==='settings')appModeOpenSettings();appModeFinishStartup(true);return}if(raw==='settings'){var settingsApplied=await appModeApply(appModeValue);if(settingsApplied===false)return;appModeOpenSettings();appModeFinishStartup(true);return}if(appModeIsReferral(raw)){var referralMode=appModeReferralMode(raw);if(referralMode){var referralApplied=await appModeApply(referralMode);if(referralApplied===false)return}else appModeSyncBack();appModeFinishStartup(true);return}var directMap={tts:'tts',voice:'tts',image:'image',explore:'explore',ai_chat:'ai_chat',stt:'stt','speech-to-text':'stt',live:'live','vexa-live':'live'};if(directMap[raw]){var directApplied=await appModeApply(directMap[raw]);if(directApplied===false)return;appModeFinishStartup(true);return}if(!raw||raw==='home'){var primaryApplied=await appModeApply(data.defaultSection||appModeValue);if(primaryApplied===false)return;appModeFinishStartup(true);return}appModeSyncBack();appModeFinishStartup(true)}

  function appModeInstallStyle(){if(document.getElementById('vexaAppModeSettingsStyle'))return;var style=document.createElement('style');style.id='vexaAppModeSettingsStyle';style.textContent='.vexa-app-settings{position:fixed;inset:0;z-index:2147483200;background:radial-gradient(circle at 50% -10%,#191919 0,#080808 42%,#040404 100%);color:#fff;direction:ltr;opacity:0;visibility:hidden;pointer-events:none;transform:translate3d(0,14px,0) scale(.994);filter:blur(5px);transition:opacity .2s ease,transform .42s cubic-bezier(.16,1,.3,1),filter .28s ease,visibility 0s linear .42s;font-family:-apple-system,BlinkMacSystemFont,"SF Pro Display","Segoe UI",sans-serif}.vexa-app-settings.open{opacity:1;visibility:visible;pointer-events:auto;transform:none;filter:none;transition-delay:0s}.vexa-app-settings-scroll{height:100%;overflow:auto;padding:calc(54px + env(safe-area-inset-top,0px)) 18px calc(28px + env(safe-area-inset-bottom,0px));box-sizing:border-box}.vexa-app-settings-inner{width:min(100%,500px);margin:0 auto;direction:ltr}.vexa-app-settings-head{margin:0 4px 24px}.vexa-app-settings-copy{opacity:0;transform:translateY(8px);transition:opacity .25s .04s ease,transform .4s .03s cubic-bezier(.16,1,.3,1)}.vexa-app-settings.open .vexa-app-settings-copy{opacity:1;transform:none}.vexa-app-settings-copy span{display:block;color:rgba(255,255,255,.32);font-size:9px;font-weight:820;letter-spacing:.14em;text-transform:uppercase}.vexa-app-settings-copy h2{margin:5px 0 0;font-size:28px;line-height:1;font-weight:850;letter-spacing:-.048em}.vexa-app-settings-section{padding:16px 14px 4px;border:1px solid rgba(255,255,255,.075);border-radius:22px;background:rgba(255,255,255,.035);box-shadow:inset 0 1px 0 rgba(255,255,255,.055),0 20px 50px rgba(0,0,0,.24);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px)}.vexa-app-settings-label{margin:0 3px 12px;opacity:0;transform:translateY(7px);transition:opacity .25s .07s ease,transform .4s .06s cubic-bezier(.16,1,.3,1)}.vexa-app-settings.open .vexa-app-settings-label{opacity:1;transform:none}.vexa-app-settings-label strong{display:block;font-size:12.5px;font-weight:820;letter-spacing:-.015em}.vexa-app-settings-label small{display:block;margin-top:3px;color:rgba(255,255,255,.4);font-size:10px;font-weight:600;line-height:1.35}.vexa-app-mode-row{width:100%;min-height:64px;padding:10px 3px;border:0;border-top:1px solid rgba(255,255,255,.065);background:transparent;color:#fff;display:flex;align-items:center;justify-content:space-between;gap:16px;text-align:left;direction:ltr;opacity:0;transform:translateY(8px);transition:opacity .25s ease,transform .38s cubic-bezier(.16,1,.3,1)}.vexa-app-settings.open .vexa-app-mode-row{opacity:1;transform:none}.vexa-app-settings.open .vexa-app-mode-row:nth-child(1){transition-delay:.1s}.vexa-app-settings.open .vexa-app-mode-row:nth-child(2){transition-delay:.13s}.vexa-app-settings.open .vexa-app-mode-row:nth-child(3){transition-delay:.16s}.vexa-app-settings.open .vexa-app-mode-row:nth-child(4){transition-delay:.19s}.vexa-app-mode-row:first-child{border-top-color:rgba(255,255,255,.085)}.vexa-app-mode-copy{min-width:0;display:block}.vexa-app-mode-copy strong{display:block;font-size:14.5px;font-weight:790;letter-spacing:-.021em}.vexa-app-mode-copy small{display:block;margin-top:3px;color:rgba(255,255,255,.38);font-size:10px;font-weight:600;line-height:1.3}.vexa-app-mode-switch{position:relative;width:42px;height:24px;flex:0 0 42px;border-radius:999px;background:#29292b;box-shadow:inset 0 0 0 1px rgba(255,255,255,.055);transition:background .22s ease,box-shadow .22s ease}.vexa-app-mode-switch i{position:absolute;width:20px;height:20px;left:2px;top:2px;border-radius:50%;background:#f4f4f4;box-shadow:0 2px 7px rgba(0,0,0,.34);transition:transform .3s cubic-bezier(.16,1,.3,1),background .22s ease}.vexa-app-mode-row[aria-checked="true"] .vexa-app-mode-switch{background:#f5f5f5;box-shadow:inset 0 0 0 1px rgba(255,255,255,.6)}.vexa-app-mode-row[aria-checked="true"] .vexa-app-mode-switch i{background:#080808;transform:translateX(18px)}.vexa-app-mode-row:disabled:not(.is-locked){opacity:.5}.vexa-app-mode-row.is-locked{cursor:not-allowed}.vexa-app-mode-row.is-locked .vexa-app-mode-copy{opacity:.66}.vexa-app-mode-lock{width:28px;height:28px;flex:0 0 28px;border-radius:50%;display:grid;place-items:center;color:#f7f7f7;background:rgba(255,255,255,.105);box-shadow:inset 0 0 0 1px rgba(255,255,255,.095),0 3px 12px rgba(0,0,0,.22)}.vexa-app-mode-lock svg{width:14px;height:14px;display:block;stroke-width:2.35}.vexa-app-settings-status{min-height:16px;margin:9px 3px 0;color:rgba(255,255,255,.34);font-size:9.5px;font-weight:650;letter-spacing:-.01em}@media(prefers-reduced-motion:reduce){.vexa-app-settings,.vexa-app-settings-copy,.vexa-app-settings-label,.vexa-app-mode-row{transition:none!important;transform:none!important;filter:none!important}}';document.head.appendChild(style)}
  ...
  function appModeRender(){var root=document.getElementById('vexaAppModeSettings');if(!root)return;root.dir='ltr';var rows=root.querySelector('#vexaAppModeRows');if(!rows)return;rows.innerHTML=appModeModes.filter(function(item){return['tts','image','explore','ai_chat','stt','live'].indexOf(item.id)>=0}).map(function(item){var active=item.id===appModeValue;var locked=!!appModeLocks[item.id];var control=locked?'<span class="vexa-app-mode-lock" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none"><rect x="5.5" y="10" width="13" height="10" rx="3" stroke="currentColor"/><path d="M8.5 10V7.5a3.5 3.5 0 0 1 7 0V10" stroke="currentColor" stroke-linecap="round"/></svg></span>':'<span class="vexa-app-mode-switch" aria-hidden="true"><i></i></span>';return '<button class="vexa-app-mode-row'+(locked?' is-locked':'')+'" data-app-mode="'+item.id+'" role="switch" aria-checked="'+(active?'true':'false')+'" aria-label="'+item.title+(locked?' (locked)':'')+'" type="button"'+((appModeSaving||locked)?' disabled':'')+'><span class="vexa-app-mode-copy"><strong>'+item.title+'</strong><small>'+item.note+'</small></span>'+control+'</button>'}).join('')}
  function appModeOpenSettings(){var root=appModeEnsureUi();if(root.classList.contains('open'))return;appModeSettingsReturn=appModeCurrent;root.classList.add('open');root.setAttribute('aria-hidden','false');appModeSyncBack();try{appModeTg&&appModeTg.HapticFeedback&&appModeTg.HapticFeedback.impactOccurred&&appModeTg.HapticFeedback.impactOccurred('light')}catch(e){}}
  function appModeCloseSettings(){var root=document.getElementById('vexaAppModeSettings');if(root){root.classList.remove('open');root.setAttribute('aria-hidden','true')}if(appModeSettingsReturn)appModeCurrent=appModeSettingsReturn;appModeSettingsReturn='';appModeSyncBack()}
  async function appModeSave(mode){mode=appModeNormalize(mode);if(appModeSaving||mode===appModeValue)return;var previous=appModeValue;appModeSaving=true;appModeValue=mode;appModeRender();var status=document.getElementById('vexaAppModeStatus');if(status)status.textContent='Saving…';try{var response=await appModeBaseFetch('/mini-app/api/app-mode',{method:'POST',headers:{'content-type':'application/json','accept':'application/json'},cache:'no-store',body:JSON.stringify({initData:appModeInitData,mode:mode})});var data=await response.json().catch(function(){return{}});if(!response.ok)throw new Error(data.error||'Could not update settings');appModeValue=appModeNormalize(data.appMode||mode);if(status)status.textContent='Updated';appModeSyncBack();try{appModeTg&&appModeTg.HapticFeedback&&appModeTg.HapticFeedback.selectionChanged&&appModeTg.HapticFeedback.selectionChanged()}catch(e){}}catch(error){appModeValue=previous;if(status)status.textContent=String(error&&error.message||'Could not update settings')}finally{appModeSaving=false;appModeRender()}}

  window.fetch=async function(input,init){var path=typeof input==='string'?input:String(input&&input.url||'');var response=await appModeBaseFetch(input,init);if(path.indexOf('/mini-app/api/session')>=0&&response.ok){try{var data=await response.clone().json();await appModeApplySession(data)}catch(e){}}return response};
  appModeBindBack();
