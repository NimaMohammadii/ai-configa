...
const APP_MODE_SETTINGS_PATCH = String.raw`
  ...
  var appModeModes=[
    {id:'tts',title:'Text to Speech',nav:'Voice',note:'AI voice from text'},
    {id:'image',title:'Image Creator',nav:'Image',note:'Create and edit images'},
    {id:'explore',title:'Explore Prompts',nav:'Explore',note:'Ready-made visual prompts'},
    {id:'ai_chat',title:'🐙 AI Chat',nav:'🐙 Chat',note:'Chat with Vexa'},
    {id:'stt',title:'Speech to Text',nav:'Text',note:'Transcribe speech and audio'},
    {id:'live',title:'Vexa Live',nav:'Live',note:'Open the live media workspace'}
  ];
  ...
  function appModeSetCurrent(mode){appModeCurrent=appModeNormalize(mode);appModeRenderNav();appModeSyncBack();requestAnimationFrame(appModeSyncBack)}
  ...
  function appModeApply(mode){mode=appModeNormalize(mode);var chatPath=window.location.pathname.indexOf('/mini-app/chat')===0;if(chatPath){if(mode==='ai_chat'){appModeSetCurrent('ai_chat');return}window.location.replace('/mini-app?section='+encodeURIComponent(mode));return}if(mode==='ai_chat'){window.location.replace('/mini-app/chat');return}appModeCloseWorkspaces(mode);if(mode==='tts'){try{if(typeof setCreationMode==='function')setCreationMode('voice');else if(document.body&&document.body.classList.contains('image-mode'))appModeClick('toggle-creation-mode')}catch(e){}appModeSetCurrent(mode);return}if(mode==='image'){try{if(typeof setCreationMode==='function')setCreationMode('image');else if(document.body&&!document.body.classList.contains('image-mode'))appModeClick('toggle-creation-mode')}catch(e){}appModeSetCurrent(mode);return}if(mode==='explore'){try{if(typeof setCreationMode==='function')setCreationMode('image');else if(document.body&&!document.body.classList.contains('image-mode'))appModeClick('toggle-creation-mode')}catch(e){}try{if(typeof openExplorePage==='function')openExplorePage();else appModeWhen('[data-action="open-explore-page"]',function(button){button.click()})}catch(e){}appModeSetCurrent(mode);return}if(mode==='stt'){appModeWhen('#speechToTextOpen',function(button){if(button.getAttribute('aria-pressed')!=='true')button.click();appModeSetCurrent('stt')});appModeSetCurrent(mode);return}if(mode==='live'){appModeWhen('#vexaLiveOpen',function(button){if(button.getAttribute('aria-pressed')!=='true')button.click();appModeSetCurrent('live')});appModeSetCurrent(mode)}}
  function appModeApplySession(data){if(!data||typeof data!=='object')return;appModeValue=appModeNormalize(data.appMode);appModeLanguage=String(data.language||appModeLanguage||'en').toLowerCase().split(/[-_]/)[0];appModeReady=true;appModeBindBack();appModeEnsureNav();appModeRender();var raw=appModeRawLaunch();var chatPath=window.location.pathname.indexOf('/mini-app/chat')===0;if(chatPath){appModeSetCurrent('ai_chat');if(raw==='settings')setTimeout(appModeOpenSettings,0);return}if(raw==='settings'){appModeApply(appModeValue);setTimeout(appModeOpenSettings,0);return}if(appModeIsReferral(raw)){var referralMode=appModeReferralMode(raw);if(referralMode)appModeSetCurrent(referralMode);else{appModeRenderNav();appModeSyncBack()}return}var directMap={tts:'tts',voice:'tts',image:'image',explore:'explore',ai_chat:'ai_chat',stt:'stt','speech-to-text':'stt',live:'live','vexa-live':'live'};if(directMap[raw]){appModeApply(directMap[raw]);return}if(!raw||raw==='home')appModeApply(appModeValue);else{appModeRenderNav();appModeSyncBack()}}

  function appModeInstallStyle(){if(document.getElementById('vexaAppModeSettingsStyle'))return;var style=document.createElement('style');style.id='vexaAppModeSettingsStyle';style.textContent='.vexa-section-nav{position:fixed;z-index:2147482000;left:10px;right:10px;top:calc(9px + env(safe-area-inset-top,0px));height:38px;display:flex;align-items:center;justify-content:center;pointer-events:auto;opacity:0;transform:translateY(-7px) scale(.985);transition:opacity .28s ease,transform .42s cubic-bezier(.16,1,.3,1);font-family:-apple-system,BlinkMacSystemFont,"SF Pro Display","Segoe UI",sans-serif}.vexa-section-nav.ready{opacity:1;transform:none}.vexa-section-nav-track{max-width:100%;display:flex;align-items:center;justify-content:flex-start;gap:5px;overflow-x:auto;overscroll-behavior-x:contain;scrollbar-width:none;padding:3px;border-radius:15px;background:rgba(7,7,7,.72);box-shadow:inset 0 0 0 1px rgba(255,255,255,.055),0 8px 28px rgba(0,0,0,.18);backdrop-filter:blur(14px) saturate(1.08);-webkit-backdrop-filter:blur(14px) saturate(1.08)}.vexa-section-nav-track::-webkit-scrollbar{display:none}.vexa-section-nav button{position:relative;height:30px;min-width:max-content;padding:0 10px;border:0;border-radius:11px;background:transparent;color:rgba(255,255,255,.54);font-size:10.5px;font-weight:760;line-height:1;letter-spacing:-.012em;white-space:nowrap;transition:background .2s ease,color .2s ease,transform .2s ease}.vexa-section-nav button:active{transform:scale(.94)}.vexa-section-nav button.primary{color:#fff;background:rgba(255,255,255,.085);padding-right:18px}.vexa-section-nav button.primary:after{content:"";position:absolute;right:8px;top:50%;width:4px;height:4px;margin-top:-2px;border-radius:50%;background:#fff;box-shadow:0 0 8px rgba(255,255,255,.45)}body.vexa-section-shell .app{padding-top:calc(66px + env(safe-area-inset-top))!important}body.vexa-section-shell #aiChatOpen,body.vexa-section-shell #modeToggle,body.vexa-section-shell #speechToTextOpen,body.vexa-section-shell #vexaLiveOpen{display:none!important}body.vexa-section-shell .ai-chat-head{top:calc(56px + env(safe-area-inset-top))!important}body.vexa-section-shell .ai-chat-messages{padding-top:calc(106px + env(safe-area-inset-top))!important}body.vexa-section-shell #explorePage{padding-top:calc(66px + env(safe-area-inset-top))!important}body.vexa-section-shell #voicesPage{padding-top:calc(48px + env(safe-area-inset-top))!important}body.vexa-settings-open .vexa-section-nav{opacity:0;transform:translateY(-8px) scale(.98);pointer-events:none}body.vexa-live-open .vexa-section-nav,body.vexa-live-open .vexa-section-nav *{pointer-events:auto!important}.vexa-app-settings{position:fixed;inset:0;z-index:2147483200;background:#050505;color:#fff;opacity:0;visibility:hidden;pointer-events:none;transform:translate3d(0,18px,0) scale(.992);filter:blur(6px);transition:opacity .22s ease,transform .46s cubic-bezier(.16,1,.3,1),filter .3s ease,visibility 0s linear .46s;font-family:-apple-system,BlinkMacSystemFont,"SF Pro Display","Segoe UI",sans-serif}.vexa-app-settings.open{opacity:1;visibility:visible;pointer-events:auto;transform:none;filter:none;transition-delay:0s}.vexa-app-settings-scroll{height:100%;overflow:auto;padding:calc(62px + env(safe-area-inset-top,0px)) 20px calc(34px + env(safe-area-inset-bottom,0px));box-sizing:border-box}.vexa-app-settings-inner{width:min(100%,520px);margin:0 auto}.vexa-app-settings-head{margin:0 2px 30px}.vexa-app-settings-copy{opacity:0;transform:translateY(10px);transition:opacity .28s .05s ease,transform .46s .04s cubic-bezier(.16,1,.3,1)}.vexa-app-settings.open .vexa-app-settings-copy{opacity:1;transform:none}.vexa-app-settings-copy span{display:block;color:rgba(255,255,255,.34);font-size:9.5px;font-weight:820;letter-spacing:.11em;text-transform:uppercase}.vexa-app-settings-copy h2{margin:6px 0 0;font-size:29px;line-height:1;font-weight:840;letter-spacing:-.045em}.vexa-app-settings-section{padding:0;background:transparent}.vexa-app-settings-label{margin:0 2px 13px;opacity:0;transform:translateY(8px);transition:opacity .28s .09s ease,transform .44s .08s cubic-bezier(.16,1,.3,1)}.vexa-app-settings.open .vexa-app-settings-label{opacity:1;transform:none}.vexa-app-settings-label strong{display:block;font-size:13px;font-weight:810;letter-spacing:-.018em}.vexa-app-settings-label small{display:block;margin-top:4px;color:rgba(255,255,255,.38);font-size:10.5px;font-weight:610;line-height:1.35}.vexa-app-mode-row{width:100%;min-height:70px;padding:12px 2px;border:0;border-top:1px solid rgba(255,255,255,.065);background:transparent;color:#fff;display:flex;align-items:center;justify-content:space-between;gap:18px;text-align:left;opacity:0;transform:translateY(9px);transition:opacity .28s ease,transform .42s cubic-bezier(.16,1,.3,1)}.vexa-app-settings.open .vexa-app-mode-row{opacity:1;transform:none}.vexa-app-settings.open .vexa-app-mode-row:nth-child(1){transition-delay:.11s}.vexa-app-settings.open .vexa-app-mode-row:nth-child(2){transition-delay:.14s}.vexa-app-settings.open .vexa-app-mode-row:nth-child(3){transition-delay:.17s}.vexa-app-settings.open .vexa-app-mode-row:nth-child(4){transition-delay:.20s}.vexa-app-settings.open .vexa-app-mode-row:nth-child(5){transition-delay:.23s}.vexa-app-settings.open .vexa-app-mode-row:nth-child(6){transition-delay:.26s}.vexa-app-mode-row:first-child{border-top-color:rgba(255,255,255,.09)}.vexa-app-mode-copy{min-width:0;display:block}.vexa-app-mode-copy strong{display:block;font-size:15px;font-weight:780;letter-spacing:-.022em}.vexa-app-mode-copy small{display:block;margin-top:4px;color:rgba(255,255,255,.36);font-size:10.5px;font-weight:610;line-height:1.3}.vexa-app-mode-switch{position:relative;width:43px;height:25px;flex:0 0 43px;border-radius:999px;background:#29292b;box-shadow:inset 0 0 0 1px rgba(255,255,255,.055);transition:background .24s ease,box-shadow .24s ease}.vexa-app-mode-switch i{position:absolute;width:21px;height:21px;left:2px;top:2px;border-radius:50%;background:#f4f4f4;box-shadow:0 2px 7px rgba(0,0,0,.34);transition:transform .32s cubic-bezier(.16,1,.3,1),background .24s ease}.vexa-app-mode-row[aria-checked="true"] .vexa-app-mode-switch{background:#f5f5f5;box-shadow:inset 0 0 0 1px rgba(255,255,255,.6)}.vexa-app-mode-row[aria-checked="true"] .vexa-app-mode-switch i{background:#080808;transform:translateX(18px)}.vexa-app-mode-row:disabled{opacity:.52}.vexa-app-settings-status{min-height:18px;margin:12px 2px 0;color:rgba(255,255,255,.34);font-size:10px;font-weight:650;letter-spacing:-.01em}.vexa-app-settings[dir="rtl"] .vexa-app-mode-row{text-align:right}.vexa-app-settings[dir="rtl"] .vexa-app-mode-switch i{left:auto;right:2px}.vexa-app-settings[dir="rtl"] .vexa-app-mode-row[aria-checked="true"] .vexa-app-mode-switch i{transform:translateX(-18px)}@media(max-width:420px){.vexa-section-nav{justify-content:flex-start}.vexa-section-nav button{padding-left:9px;padding-right:9px}.vexa-section-nav button.primary{padding-right:17px}}@media(prefers-reduced-motion:reduce){.vexa-section-nav,.vexa-app-settings,.vexa-app-settings-copy,.vexa-app-settings-label,.vexa-app-mode-row{transition:none!important;transform:none!important;filter:none!important}}';document.head.appendChild(style)}
  function appModeEnsureNav(){appModeInstallStyle();var nav=document.getElementById('vexaPrimaryNav');if(!nav){nav=document.createElement('nav');nav.id='vexaPrimaryNav';nav.className='vexa-section-nav';nav.setAttribute('aria-label','Vexa sections');nav.innerHTML='<div id="vexaPrimaryNavTrack" class="vexa-section-nav-track"></div>';document.body.appendChild(nav);nav.addEventListener('click',function(event){var button=event.target&&event.target.closest?event.target.closest('[data-vexa-section]'):null;if(!button)return;var mode=button.getAttribute('data-vexa-section');if(mode)appModeApply(mode)})}document.body.classList.add('vexa-section-shell');requestAnimationFrame(function(){nav.classList.add('ready')});appModeRenderNav();return nav}
  function appModeRenderNav(){var nav=document.getElementById('vexaPrimaryNav');if(!nav)return;var track=nav.querySelector('#vexaPrimaryNavTrack');if(!track)return;track.innerHTML=appModeModes.filter(function(item){return item.id!==appModeCurrent}).map(function(item){var primary=item.id===appModeValue;return '<button type="button" data-vexa-section="'+item.id+'" class="'+(primary?'primary':'')+'" aria-label="Open '+item.title+'">'+item.nav+'</button>'}).join('')}
  ...
  function appModeRender(){var root=document.getElementById('vexaAppModeSettings');if(root){root.dir=['fa','ar'].indexOf(appModeLanguage)>=0?'rtl':'ltr';var rows=root.querySelector('#vexaAppModeRows');if(rows)rows.innerHTML=appModeModes.map(function(item){var active=item.id===appModeValue;return '<button class="vexa-app-mode-row" data-app-mode="'+item.id+'" role="switch" aria-checked="'+(active?'true':'false')+'" type="button"'+(appModeSaving?' disabled':'')+'><span class="vexa-app-mode-copy"><strong>'+item.title+'</strong><small>'+item.note+'</small></span><span class="vexa-app-mode-switch" aria-hidden="true"><i></i></span></button>'}).join('')}appModeRenderNav()}
  function appModeOpenSettings(){var root=appModeEnsureUi();if(root.classList.contains('open'))return;appModeSettingsReturn=appModeCurrent;document.body.classList.add('vexa-settings-open');root.classList.add('open');root.setAttribute('aria-hidden','false');appModeSyncBack();try{appModeTg&&appModeTg.HapticFeedback&&appModeTg.HapticFeedback.impactOccurred&&appModeTg.HapticFeedback.impactOccurred('light')}catch(e){}}
  function appModeCloseSettings(){var root=document.getElementById('vexaAppModeSettings');if(root){root.classList.remove('open');root.setAttribute('aria-hidden','true')}document.body.classList.remove('vexa-settings-open');if(appModeSettingsReturn)appModeCurrent=appModeSettingsReturn;appModeSettingsReturn='';appModeRenderNav();appModeSyncBack()}
  async function appModeSave(mode){mode=appModeNormalize(mode);if(appModeSaving||mode===appModeValue)return;var previous=appModeValue;appModeSaving=true;appModeValue=mode;appModeRender();var status=document.getElementById('vexaAppModeStatus');if(status)status.textContent='Saving…';try{var response=await appModeBaseFetch('/mini-app/api/app-mode',{method:'POST',headers:{'content-type':'application/json','accept':'application/json'},cache:'no-store',body:JSON.stringify({initData:appModeInitData,mode:mode})});var data=await response.json().catch(function(){return{}});if(!response.ok)throw new Error(data.error||'Could not update settings');appModeValue=appModeNormalize(data.appMode||mode);if(status)status.textContent='Updated';appModeRenderNav();appModeSyncBack();try{appModeTg&&appModeTg.HapticFeedback&&appModeTg.HapticFeedback.selectionChanged&&appModeTg.HapticFeedback.selectionChanged()}catch(e){}}catch(error){appModeValue=previous;if(status)status.textContent=String(error&&error.message||'Could not update settings')}finally{appModeSaving=false;appModeRender()}}
