...
async function enhanceSession(response, request, env) {
  ...
  return json({ ...data, appMode, appModeLocks, defaultSection: appMode, imagePricing: dynamicPricingPayload(0) }, response.status);
}
...
const APP_MODE_SETTINGS_PATCH = String.raw`
  ...
  function appModeRender(){var root=document.getElementById('vexaAppModeSettings');if(!root)return;root.dir=['fa','ar'].indexOf(appModeLanguage)>=0?'rtl':'ltr';var rows=root.querySelector('#vexaAppModeRows');if(!rows)return;rows.innerHTML=appModeModes.filter(function(item){return['tts','image','ai_chat','stt','live'].indexOf(item.id)>=0}).map(function(item){var active=item.id===appModeValue;var locked=!!appModeLocks[item.id];var control=locked?'<span class="vexa-app-mode-lock" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none"><rect x="5.5" y="10" width="13" height="10" rx="3" stroke="currentColor"/><path d="M8.5 10V7.5a3.5 3.5 0 0 1 7 0V10" stroke="currentColor" stroke-linecap="round"/></svg></span>':'<span class="vexa-app-mode-switch" aria-hidden="true"><i></i></span>';return '<button class="vexa-app-mode-row'+(locked?' is-locked':'')+'" data-app-mode="'+item.id+'" role="switch" aria-checked="'+(active?'true':'false')+'" aria-label="'+item.title+(locked?' (locked)':'')+'" type="button"'+((appModeSaving||locked)?' disabled':'')+'><span class="vexa-app-mode-copy"><strong>'+item.title+'</strong><small>'+item.note+'</small></span>'+control+'</button>'}).join('')}
  ...
  window.fetch=async function(input,init){var path=typeof input==='string'?input:String(input&&input.url||'');if(path.indexOf('/mini-app/api/session')>=0&&init&&typeof init.body==='string'){try{var sessionBody=JSON.parse(init.body);sessionBody.launchSection=appModeRawLaunch();init=Object.assign({},init,{body:JSON.stringify(sessionBody)})}catch(e){}}var response=await appModeBaseFetch(input,init);if(path.indexOf('/mini-app/api/session')>=0&&response.ok){try{var data=await response.clone().json();await appModeApplySession(data)}catch(e){}}return response};
  appModeBindBack();
