...
function callbackPage(message, success, installUrl = "") {
  ...
  const installButton = installUrl
    ...
    : "";
  const icon = success ? "✓" : "!";
  return new Response(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#000"><title>GitHub connection</title><style>html,body{margin:0;min-height:100%;background:#000;color:#fff;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif}body{display:grid;place-items:center;padding:24px;box-sizing:border-box}.card{width:min(420px,100%);padding:30px 24px;border:1px solid #242424;border-radius:24px;background:#0b0b0c;text-align:center}.icon{width:52px;height:52px;margin:0 auto 18px;border-radius:50%;display:grid;place-items:center;background:${success ? "#fff" : "#2a1515"};color:${success ? "#000" : "#ff9b9b"};font-size:26px;font-weight:700}h1{font-size:21px;margin:0 0 10px}p{color:#aaa;line-height:1.5;margin:0 0 22px}.button,button{display:block;width:100%;box-sizing:border-box;border:0;border-radius:14px;background:#fff;color:#000;padding:14px 18px;font:600 15px inherit;text-decoration:none;margin-top:10px;cursor:pointer}.secondary{background:#1b1b1d;color:#fff}</style></head><body><main class="card"><div class="icon">${icon}</div><h1>${success ? "GitHub connected" : "Connection failed"}</h1><p>${safeMessage}</p>${installButton}<button class="secondary" onclick="window.close()">Return to AI Chat</button></main></body></html>`, {
